...
| A conversation does not survive its engine | Found 2026-09-24 by driving the migrated build. The engine holds only the conversations it created, so one from an earlier engine can be replayed but not acted on — `unknown-conversation`. **Resuming is not a missing function, it is missing data:** `conversation.created` records `projectId` and `title` and neither the participants nor the profile. Participants are only partly recoverable from event actors, and a participant who never spoke leaves no trace — `01a0d1ae` was created with claude, codex and deepseek and shows no `codex` at all. `profileId` appears only on `policy.changed`, so a conversation that never changed it has no record of what it ran under. Milestone 4's "conversation history/reopen" is therefore a payload change and a core release, not a UI feature, and existing conversations stay display-only because their data was never written. Made honest in the meantime: the replay batch carries `active` and the extension offers a fresh conversation instead of letting a message fail into a dead room |
