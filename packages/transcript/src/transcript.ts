 ...
 *
 * **`payload` is deliberately loose, and tightening it is a mistake.** The log
 * writes it as an object and this boundary reads it defensively, so an
 * unreadable one becomes an empty list rather than a crash mid-render — see the
 * note on `questionFields`. Typing it as the protocol's discriminated payload
 * would claim a guarantee the log does not make: an event written by an older
 * build would then fail to typecheck against a newer union, which is a
 * compile-time lie about runtime data that has already been written down.
 * (This was tried, and the upstream tests rejected it — they build payloads with
 * no `type` key, which is what a real old event looks like.)
 */
